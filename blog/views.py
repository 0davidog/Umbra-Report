...
class ReportList(generic.ListView):
    ...
    Attributes:
        queryset (QuerySet):
        The queryset of reports to be displayed, filtered by status=1 (active).
        template_name (str):
        The name of the template used to render the list of reports.
        paginate_by (int):
        The number of reports to display per page, pagination is enabled.

    Note:
        This view inherits from Django's generic ListView class,
...
def full_report(request, slug):
    ...
    Returns:
        HttpResponse:
        The HTTP response containing the rendered full_report.html template
        with report details and comments.
    ...
    if request.method == "POST":
        ...
        # save the comment and display a success message
    ...
    # Render the full_report.html template
    # with the report details, comments, and comment form
    return render(
        request,
...
def edit_comment(request, slug, comment_id):
    ...
    Returns:
        ...
        Redirects to the full_report view after processing the comment update.
    ...
    if request.method == "POST":
        ...
        # Retrieve the comment object based on the provided comment_id,
        # or return a 404 error if not found
        comment = get_object_or_404(Comment, pk=comment_id)
        # Create a comment form instance with the submitted data
        # and the existing comment instance
        comment_form = CommentForm(data=request.POST, instance=comment)
        # Check if the comment form is valid
        # and if the current user is the author of the comment
...
def comment_delete(request, slug, comment_id):
    ...
    Returns:
        HttpResponseRedirect:
        Redirects to the full_report view
        after processing the comment deletion.
...
def create_report(request):
    ...
    Returns:
        HttpResponse:
        The HTTP response containing the rendered create_report.html template
    ...
    if request.method == "POST":
        ...
        # If the report form is valid,
        # save the report and display a success message
...
def like_report(request, slug):
    """
    View for adding likes to a report.
    Related to :model: `Report`
    """

    # Retrieve the report object with the given slug from the database
    # or return a 404 error if not found
    post = get_object_or_404(Report, slug=slug)
    # Check if the user has already liked the report
    if post.likes.filter(id=request.user.id).exists():
        # If the user has already liked the report, remove the user's like
        post.likes.remove(request.user)
    else:
        # If the user has not liked the report yet, add the user's like
        post.likes.add(request.user)

    # Redirect the user to the page displaying the full report
    # after processing the like
    return HttpResponseRedirect(reverse('full_report', args=[slug]))
...
def edit_report(request, slug):
    ...
    Returns:
        HttpResponse:
        The HTTP response containing the rendered edit_report.html template
        with the report form.
    ...
    # If the request method is POST, process the form submission
    if request.method == "POST":
        # Create a report form instance with the submitted data,
        # files, and the instance of the report being edited
        ...
        # If the request method is not POST,
        # create a report form instance with the instance
        # of the report being edited
...
def delete_report(request, slug, report_id):
    ...
    Returns:
        HttpResponseRedirect:
        Redirects to the home page after processing the report deletion.

    """
    # Retrieve the report object based on the provided slug
    # and report ID, or return a 404 error if not found
...
def about_site(request):
    ...
    Renders instance of :model: `About`
    ...
    Returns:
        HttpResponse:
        The HTTP response containing the rendered about.html template
        with the about object.
...
def user_profile(request, author):
    ...
    Returns:
        HttpResponse:
        The HTTP response containing the rendered user_profile.html template
